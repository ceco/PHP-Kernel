<?
    ...
    function index_index (){
        ...
        print_r( $apps );
        foreach( $apps as $app )
            print_r( $kernel->list_objects( $app ) );
...
class base_module {
    ...
    function base_module($obj) {
        $this->kernel = $obj; // keep a copy of the module names
    ...
    function show_buttons($vbox) {
        ...
        foreach($this->kernel->modules as $module) {
            ...
            $hbox->pack_start(new GtkLabel());
            $button->connect('clicked', array(&$this->kernel,'on_clicked'), $module); // event handler to handle button click
        }
    }
}
