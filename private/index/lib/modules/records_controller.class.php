    ...
    class records_controller {
        ...
        */
        public function mode_view (){
            ...
            if( $row = $this->getOne( $id ) ){
                $this->kernel->params_html = array_merge( $this->kernel->params_html, $row );
        ...
        */
        public function getAll (){
            ...
            $list = $this->kernel->db->getAll("SELECT * FROM {$this->table} WHERE {$this->filter["where"]} {$this->filter["group_by"]} {$this->filter["order_by"]} LIMIT {$this->filter["start"]}, {$this->filter["per_page"]}");
